...
This report details time series analysis of sales within TRADE+ customer group. The data are first sourced from the Redpoint:
...
These four plots represent the original data, the trend, the seasonality and the variance. These are important to consider as we commonly assume in many time series techniques (including ARIMA) is that the time series is stationary. This allows preserving model stability and provides a framework to 

A stationary time series is one whose statistical properties such as mean, variance and correlation are all constant over time (i.e. to create the stationary time series, we need to minimise the trend, seasonality and variance). To 
